...
| Published file scope                            | `@news-ui/tokens` ships only `dist/` (its build output); `@news-ui/ui` ships `src/` minus `*.test.js`/`*.stories.js`/`test-setup/` (via `!`-negation entries in package.json's `files` array)     | Consumers need the compiled tokens and the component source (this library ships unbundled ES modules, not a bundle), not the test suite or Storybook stories. A `.npmignore` was tried first; verified with `npm pack --dry-run` that npm silently did not honor it at all, so switched to `files` negation instead, which did. |
